...
import {
  Shield, School, BookOpen, HelpCircle, LogOut, LayoutDashboard, Database, Activity, Loader2, UserPlus,
} from "lucide-react";
import { cn } from "@verse/ui";
import EnergyBackground from "@verse/arena-web/components/ui/EnergyBackground";
import { useRouter } from "next/navigation";
import useAuth from "@verse/arena-web/hooks/useAuth";
import { usePlatformStats } from "@verse/arena-web/hooks/usePlatformStats";
...
import InstructorsModule from "./modules/InstructorsModule";

type Tab = "overview" | "schools" | "courses" | "instructors" | "questions";
...
export default function AdminDashboard() {
  ...
  const renderActiveModule = () => {
    switch (activeTab) {
      case "overview": return <OverviewModule onNavigate={setActiveTab} stats={stats} />;
      ...
      default: return <OverviewModule onNavigate={setActiveTab} stats={stats} />;
    }
  };
        ...
        <aside className="hidden lg:flex w-72 border-r border-white/10 bg-black/60 backdrop-blur-xl flex-col shrink-0 z-50">
          <div className="p-8">
            ...
            <nav className="space-y-2">
              ...
              <AdminNavLink icon={<UserPlus size={18} />} label="Instructors" active={activeTab === "instructors"} onClick={() => setActiveTab("instructors")} />
          ...
          <header className="h-20 border-b border-white/5 px-8 flex items-center justify-between bg-black/20 shrink-0 z-40">
            ...
            <div className="flex items-center gap-6">
              <div className="hidden xl:flex items-center gap-8 pr-8 border-r border-white/10">
                <StatMinimal label="Schools" value={String(stats?.schoolCount ?? "—")} />
                <StatMinimal label="Fighters" value={String(stats?.userCount ?? "—")} />
                <StatMinimal label="Assets" value={String(stats?.questionCount ?? "—")} />
              </div>
              <div className="flex items-center gap-3 pl-2">
                <div className="text-right hidden sm:block">
                  <p className="text-[10px] font-mono text-white leading-none uppercase font-bold">{user?.username || "Architect"}</p>
