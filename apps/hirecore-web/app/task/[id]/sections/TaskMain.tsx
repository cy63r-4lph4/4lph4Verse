        ...
        <CardContent className="space-y-6 p-6">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InfoRow icon={<MapPin className="w-5 h-5 text-blue-400" />} label="Location" value={task.location || "Not specified"} />
              <InfoRow icon={<Clock className="w-5 h-5 text-green-400" />} label="Time Estimate" value={task.timeEstimate || "Not specified"} />
              <InfoRow icon={<DollarSign className="w-5 h-5 text-yellow-400" />} label="Budget" value={<span className="core-token font-bold">{task.budget} CØRE</span>} />
              <InfoRow
                icon={(task.serviceType === "on-site" ? <Home className="w-5 h-5 text-purple-400" /> : <Building className="w-5 h-5 text-purple-400" />)}
                label="Service Type"
            ...
            <section>
              ...
              <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                <div className="flex items-center space-x-2 text-blue-400">
                  ...
                  <span className="font-mono text-sm">{task.coordinates.lat}, {task.coordinates.lng}</span>
                </div>
                <p className="text-gray-300 text-sm mt-2">Use these coordinates for precise navigation to the task location.</p>
              </div>
            </section>
          )}
        </CardContent>
