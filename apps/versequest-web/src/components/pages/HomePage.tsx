...
import { Trophy, MessageSquare, Vote, Clock, Users, Star, Zap, Brain, Sparkles, Crown, Flame, Eye } from 'lucide-react';
import { mockQuests, mockQuestRooms, mockProposals, mockUserProfile, mockLeaderboard, getRankByAura } from '@/data/mockData';
...
export const HomePage: React.FC<HomePageProps> = ({ onTabChange }) => {
  ...
  const { userAura, isAdmin } = useContract();
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'wisdom': return <Brain className="h-4 w-4" />;
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'wisdom': return 'text-quest-wisdom border-quest-wisdom/30 bg-quest-wisdom/10';
      case 'discovery': return 'text-quest-discovery border-quest-discovery/30 bg-quest-discovery/10';
      case 'mastery': return 'text-quest-mastery border-quest-mastery/30 bg-quest-mastery/10';
  ...
  return (
    <div className="space-y-12">
          ...
          {account ? (
            <div className="flex flex-col lg:flex-row items-center justify-center gap-12 mt-12">
              ...
              <div className="text-center lg:text-left space-y-4">
                <h3 className="text-3xl font-mystic text-gradient-primary">
                  Your Aura Shines <span className="text-gradient-radiant">{getRankByAura(userAura || mockUserProfile.aura).name}</span>
                </h3>
                <div className="grid grid-cols-2 gap-6 text-lg font-tech">
                  <div className="glass-card p-4">
                    <span className="text-muted-foreground">Dragon Streak:</span>
                    <div className="font-bold text-2xl text-gradient-core">{mockUserProfile.streak} days</div>
                  </div>
                  <div className="glass-card p-4">
                    <span className="text-muted-foreground">Reflections:</span>
                    <div className="font-bold text-2xl text-gradient-primary">{mockUserProfile.reflectionCount}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-12">
              <p className="text-xl mb-8 opacity-90 font-tech">
                Connect your wallet to begin your journey of reflection and growth in the 4lph4Verse
              </p>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'Active Quests', value: activeQuests.length, icon: Trophy, color: 'core', gradient: 'gradient-core' },
          { label: 'Reflection Halls', value: mockQuestRooms.length, icon: MessageSquare, color: 'primary', gradient: 'gradient-primary' },
          { label: 'Council Proposals', value: activeProposals.length, icon: Vote, color: 'radiant', gradient: 'gradient-radiant' },
          { label: 'Awakened Minds', value: '2.8k', icon: Users, color: 'primary', gradient: 'gradient-aura' }
        ].map((stat, index) => (
          <Card key={index} className="glass-card aura-glow">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-tech text-muted-foreground">{stat.label}</p>
                  <p className={`text-3xl font-bold font-tech text-gradient-${stat.color === 'core' ? 'core' : stat.color === 'radiant' ? 'radiant' : 'primary'}`}>
                    {stat.value}
                  </p>
                </div>
                <div className={`p-4 rounded-2xl bg-${stat.gradient} shadow-${stat.color}`}>
                  <stat.icon className={`h-8 w-8 text-${stat.color}-foreground`} />
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="flex items-center justify-between">
            <h2 className="text-4xl font-mystic text-gradient-primary">Active Quests</h2>
            <Button 
              variant="outline" 
              size="sm"
              onClick={() => onTabChange('quests')}
          ...
          <div className="space-y-6">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <Badge variant="outline" className={`text-sm capitalize font-tech ${getCategoryColor(quest.category)}`}>
                          {quest.category}
                        </Badge>
                      ...
                      <CardTitle className="text-2xl font-tech font-bold">{quest.title}</CardTitle>
            ...
            <div className="pt-6">
              <h3 className="text-xl font-tech font-semibold mb-4 text-muted-foreground">Recently Completed</h3>
              {completedQuests.slice(0, 2).map((quest) => (
                <Card key={quest.id} className="glass-card mb-4 opacity-75 hover:opacity-100 transition-opacity">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline" className="border-core/30 text-core font-tech">
                        View Results
                      </Badge>
                    </div>
                  </CardContent>
          ...
          <Card className="glass-card aura-glow">
            ...
            <CardContent className="space-y-4">
              {mockLeaderboard.slice(0, 5).map((entry, index) => (
                <div key={entry.address} className="flex items-center gap-4 p-3 rounded-xl glass-card">
                  <div className={`
                    w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold font-tech
                    ${index === 0 ? 'bg-gradient-radiant text-radiant-foreground' :
                      index === 1 ? 'bg-gradient-core text-core-foreground' :
                      index === 2 ? 'bg-gradient-primary text-primary-foreground' :
          ...
          <Card className="glass-card">
            ...
            <CardContent className="space-y-4">
              {mockQuestRooms.map((room) => (
                <div key={room.id} className="p-4 rounded-xl glass-card hover:shadow-glass transition-all cursor-pointer">
                  <h4 className="font-medium text-sm font-tech mb-2">{room.questTitle}</h4>
                  <div className="flex items-center justify-between text-xs text-muted-foreground font-tech">
                    ...
                    <span className="text-gradient-core">{room.totalTips} CØRE tips</span>
                  </div>
                </div>
              ))}
              <Button 
          ...
          <Card className="glass-card shadow-radiant">
            ...
            <CardContent className="space-y-4">
              {mockProposals.slice(0, 2).map((proposal) => (
                <div key={proposal.id} className="p-4 rounded-xl bg-radiant/5 border border-radiant/20">
                  <h4 className="font-medium text-sm font-tech mb-3">{proposal.title}</h4>
                  ...
                  <div className="mt-3 h-2 bg-secondary rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ 
                        width: `${(parseFloat(proposal.yesVotes.replace(',', '')) / 
                                  (parseFloat(proposal.yesVotes.replace(',', '')) + 
                                   parseFloat(proposal.noVotes.replace(',', '')))) * 100}%` 
                      }}
                    />
                  </div>
                </div>
