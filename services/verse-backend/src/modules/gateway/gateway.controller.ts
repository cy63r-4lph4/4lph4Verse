...
export class GatewayController {
  ...
  async getSectors(@Request() req) {
    const sectors = await this.gatewayService.getDiscoverableSectors(
      req.user.id,
    );
    return sectors;
  }
  ...
  @Post('join-sector')
  @UseGuards(JwtAuthGuard)
  async joinSector(@Body() body: JoinSectorDto, @Request() req) {
    return await this.gatewayService.joinSectorWrapper(
      req.user.id,
      body.accessKey,
    );
  }
  ...
  async changePassword(
    @Body() body: import('./dto/changePassword').ChangePasswordDto,
    @Request() req,
  ) {
    return await this.gatewayService.changePassword(req.user.id, body);
  }
}
