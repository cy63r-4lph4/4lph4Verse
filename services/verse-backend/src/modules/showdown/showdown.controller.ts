...
export class ShowdownController {
  ...
  async create(@Body() body: CreateShowdownDto, @Request() req) {
    ...
    if (
      !course ||
      (arenaUser.role !== 'admin' && course.schoolId !== arenaUser.schoolId)
    ) {
      throw new ForbiddenException(
        'Cannot create a showdown for a course outside your school.',
      );
    }
  ...
  async buildBracket(
    ...
    const arenaUser = await this.identity.resolve(req.user.id);
    return this.showdownService.buildBracket(
      id,
      arenaUser.id,
      body.arenaUserIds,
    );
  ...
  async challenge(@Body() body: CreateDuelChallengeDto, @Request() req) {
    const arenaUser = await this.identity.resolve(req.user.id);
    const showdown = await this.showdownService.createDuelChallenge(
      arenaUser.id,
      body,
    );
  ...
  @UsePipes(new ValidationPipe({ transform: true }))
  async createAsyncChallenge(
    @Body() body: CreateDuelChallengeDto,
    @Request() req,
  ) {
    const arenaUser = await this.identity.resolve(req.user.id);
    const showdown = await this.showdownService.createAsyncDuelChallenge(
      arenaUser.id,
      body,
    );
  ...
  async submitAsyncAnswers(
    ...
    const arenaUser = await this.identity.resolve(req.user.id);
    return this.showdownService.submitAsyncAnswers(
      id,
      arenaUser.id,
      body.answers,
    );
  ...
  async getAsyncDuelState(@Param('id') id: string, @Request() req) {
    ...
    const pA = state.participants.find(
      (p) => p.id === state.matches[0]?.playerAId,
    );
    const pB = state.participants.find(
      (p) => p.id === state.matches[0]?.playerBId,
    );
    ...
    if (!bothDone) {
      ...
      const myId = state.participants.find(
        (p) => p.arenaUserId === arenaUser.id,
      )?.id;
  ...
  async searchOpponents(
    ...
    const arenaUser = await this.identity.resolve(req.user.id);
    return this.showdownService.searchCourseMembers(
      courseId,
      arenaUser.id,
      query,
    );
