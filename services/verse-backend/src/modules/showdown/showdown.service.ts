...
export class ShowdownService {
  ...
  async buildBracket(
    ...
    return this.db.transaction(async (tx) => {
      await tx
        ...
        .values(
          arenaUserIds.map((arenaUserId) => ({ showdownId, arenaUserId })),
        );
      ...
      await tx.insert(schema.showdownMatches).values(
        matches.map((m) => ({
          ...
          status:
            m.playerBId === null ? ('complete' as const) : ('pending' as const),
        })),
      );
  ...
  async startMatch(
    ...
    const endsAt = new Date(
      startedAt.getTime() + showdown.timeLimitSeconds * 1000,
    );
  ...
  async resolveQuestion(
    ...
    const isLastQuestion =
      match.questionsCompleted + 1 >= showdown.questionsPerMatch;
  ...
  async listIncomingChallenges(arenaUserId: string) {
    ...
    return participations
      .filter(
        (p) =>
          p.showdown.mode === 'duel' &&
          p.showdown.status === 'challenge_pending',
      )
      .filter((p) => p.showdown.createdBy !== arenaUserId)
      .map((p) => {
        const challenger = p.showdown.participants.find(
          (x) => x.arenaUserId === p.showdown.createdBy,
        );
        return {
  ...
  async listOutgoingChallenges(arenaUserId: string) {
    ...
    return rows.map((r) => ({
      ...
      courseId: r.courseId,
      toUsername:
        r.participants.find((p) => p.arenaUserId !== arenaUserId)?.arenaUser
          .user.username ?? '?',
  ...
  async acceptDuelChallenge(showdownId: string, requesterArenaUserId: string) {
    ...
    if (showdown.mode !== 'duel' || showdown.status !== 'challenge_pending') {
      throw new BadRequestException(
        'This challenge is not awaiting a response.',
      );
    }
  ...
  async startDuelQuestion(showdownId: string, matchId: string) {
    ...
    const [mq] = await this.db
      .insert(schema.showdownMatchQuestions)
      .values({
        matchId,
        questionId: question.id,
        questionNumber: match.questionsCompleted + 1,
  ...
  async getFeed(courseId: string, viewerArenaUserId: string) {
    ...
    const battles = await Promise.all(
      recentCompleted
        ...
        .map(async (s) => {
          ...
          const scores = lastMatch
            ? await this.computeMatchScores(lastMatch.id)
            : {};
    ...
    const liveDuels = pending
      ...
      .filter(
        (p) =>
          (p.showdown.mode === 'duel' || p.showdown.mode === 'async_duel') &&
          (p.showdown.status === 'live' ||
            p.showdown.status === 'ready_check') &&
          !p.completedAt,
      )
      .map((p) => {
        const opponent = p.showdown.participants.find(
          (x) => x.arenaUserId !== viewerArenaUserId,
        );
        return {
          id: p.showdown.id,
          showdownId: p.showdown.id,
    ...
    const publicActivity = activePublicDuels.map((s) => {
      const challenger = s.participants.find(
        (p) => p.arenaUserId === s.createdBy,
      );
      const opponent = s.participants.find(
        (p) => p.arenaUserId !== s.createdBy,
      );
  ...
  async getFullState(showdownId: string) {
    ...
    const matches = await this.db.query.showdownMatches.findMany({
      where: (m, { eq }) => eq(m.showdownId, showdownId),
      orderBy: [
        asc(schema.showdownMatches.round),
        asc(schema.showdownMatches.matchIndex),
      ],
  ...
  async listTournamentsForCourse(courseId: string, statusFilter?: string) {
    const rows = await this.db.query.showdowns.findMany({
      where: (s, { eq, and }) =>
        and(eq(s.courseId, courseId), eq(s.mode, 'tournament')),
      orderBy: (s, { desc }) => [desc(s.createdAt)],
    ...
    const mapped = rows.map((s) => {
      const maxRound =
        s.matches.length > 0 ? Math.max(...s.matches.map((m) => m.round)) : 0;
  ...
  async resetTournament(showdownId: string, requesterArenaUserId: string) {
    ...
    return this.db.transaction(async (tx) => {
      ...
      // whole bracket tree.
      await tx
        .delete(schema.showdownParticipants)
        .where(eq(schema.showdownParticipants.showdownId, showdownId));
  ...
  private async computeMatchScores(
    matchId: string,
  ...
  private async pickUnusedQuestionFor(showdownId: string) {
    ...
    return this.questionsService.pickUnused(
      showdown.courseId,
      used.map((u) => u.questionId),
    );
  }
  ...
  private async getMatchOrThrow(matchId: string, showdownId: string) {
    const match = await this.db.query.showdownMatches.findFirst({
      where: (m, { eq, and }) =>
        and(eq(m.id, matchId), eq(m.showdownId, showdownId)),
    });
    ...
    if (showdown.createdBy !== requesterArenaUserId) {
      ...
      if (requester?.role !== 'admin') {
        throw new ForbiddenException(
          'Only the creator or an admin can control this showdown.',
        );
      }
    }
    return showdown;
  ...
  async createAsyncDuelChallenge(
    initiatorArenaUserId: string,
    dto: {
      courseId: string;
      opponentArenaUserId: string;
  ...
  async submitAsyncAnswers(
    ...
    const participant = await this.db.query.showdownParticipants.findFirst({
      where: (p, { eq, and }) =>
        and(eq(p.showdownId, showdownId), eq(p.arenaUserId, arenaUserId)),
    });
    ...
    return this.db.transaction(async (tx) => {
      ...
      await tx
        .update(schema.showdownParticipants)
      ...
      if (pA?.completedAt && pB?.completedAt) {
        ...
        const winnerId =
          scoreA > scoreB ? pA.id : scoreB > scoreA ? pB.id : null;
    ...
    const members = await this.db.query.arenaUser.findMany({
      where: (u, { eq }) =>
        eq(
          u.schoolId,
          this.db
            .select({ schoolId: schema.arenaCourses.schoolId })
  ...
  async getAsyncDuelsList(courseId: string, arenaUserId: string) {
    const duels = await this.db.query.showdowns.findMany({
      where: (s, { eq, and }) =>
        and(eq(s.courseId, courseId), eq(s.mode, 'async_duel')),
      with: {
        participants: { with: { arenaUser: { with: { user: true } } } },
        matches: true,
      },
    ...
    return duels.filter((d) =>
      d.participants.some((p) => p.arenaUserId === arenaUserId),
    );
  }
