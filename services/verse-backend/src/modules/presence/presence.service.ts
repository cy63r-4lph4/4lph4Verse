...
export class PresenceService implements OnModuleDestroy {
  ...
  async heartbeat(courseId: string, arenaUserId: string, username: string) {
    ...
    await this.redis.set(
      onlineKey(courseId, arenaUserId),
      payload,
      'EX',
      ONLINE_TTL_SECONDS,
    );
    await this.redis.set(
      recentKey(courseId, arenaUserId),
      payload,
      'EX',
      RECENT_TTL_SECONDS,
    );
  ...
  async listPresence(
    ...
    { arenaUserId: string; username: string; status: 'online' | 'recent' }[]
  > {
    ...
    const deadIds = memberIds.filter(
      (id) => !alive.some((a) => a.arenaUserId === id),
    );
    if (deadIds.length > 0)
      await this.redis.srem(courseSetKey(courseId), ...deadIds);
