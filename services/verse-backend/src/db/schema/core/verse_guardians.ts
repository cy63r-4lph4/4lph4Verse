...
export const verseGuardians = pgTable(
  "verse_guardians",
  {
    ...
    verseId: varchar("verse_id", { length: 255 }).notNull().references(() => verseProfiles.id, { onDelete: 'cascade' }),
  ...
  (table) => ({
    ...
    unq_guardian_per_verse: uniqueIndex("unq_guardian_per_verse").on(table.verseId, table.guardianAddress, table.epoch)
  })
);
